...
export default function SelectionFlashOverlay() {
  const [color, setColor] = useState<string>("rgba(255,255,255,0)");
  ...
  useEffect(() => {
    ...
    window.addEventListener("bg-gradient:flash", onFlash as EventListener);
    return () => {
      window.removeEventListener("bg-gradient:flash", onFlash as EventListener);
    };
  }, []);
